...
class ElementBrowser implements \TYPO3\CMS\Core\ElementBrowser\ElementBrowserHookInterface
{
    ...
    public function modifyMenuDefinition($menuDefinition)
    {
        ...
        foreach ($tabs as $key => $tabConfig) {
            ...
            $addPassOnParams = \TYPO3\CMS\Core\Utility\GeneralUtility::implodeArrayForUrl(
                '',
                \TYPO3\CMS\Core\Utility\GeneralUtility::explodeUrl2Array($addPassOnParams),
                '',
                true
            );
            $menuDefinition[$key]['addParams'] = 'onclick="jumpToUrl(\'?act=' . $key .
    ...
    private function _checkConfigAndGetDefault()
    {
        ...
        if ($this->pObj->mode == 'rte') {
            ...
            $RTEsetup = $GLOBALS['BE_USER']->getTSConfig(
                'RTE',
                \TYPO3\CMS\Backend\Utility\BackendUtility::getPagesTSconfig($RTEtsConfigParts[5])
            );
            $this->pObj->thisConfig = \TYPO3\CMS\Backend\Utility\BackendUtility::RTEsetup(
                $RTEsetup['properties'],
                $RTEtsConfigParts[0],
                $RTEtsConfigParts[2],
                $RTEtsConfigParts[4]
