...
class Handler
{
    ...
    public function main(
        ...
        list ($recordTableName, $recordUid) = \TYPO3\CMS\Core\Utility\GeneralUtility::trimExplode(
            ':',
            $linkHandlerValue
        );
        ...
        if ($this->isRecordLinkable($recordTableName, $typoScriptConfiguration, $recordArray)) {
            ...
            $currentLinkConfigurationArray = $this->mergeTypoScript(
        ...
        if (isset($cache[$parameterHash])) {
            ...
            if (is_array($record)) {
                ...
                $l18nPointer = array_key_exists('transOrigPointerField', $GLOBALS['TCA'][$recordTableName]['ctrl'])
                    ...
                    : '';
                if (
                    isset($record[$l18nPointer])
                    && intval($record[$l18nPointer]) > 0
                    && intval($record['sys_language_uid']) > 0
                ) {
     ...
     * @return void
     */
    protected function updateParentLastTypoLinkMember(
        \TYPO3\CMS\Frontend\ContentObject\ContentObjectRenderer $contentObjectRenderer
    ) {
     ...
     * @return array
     */
    protected function mergeTypoScript(
        array $linkConfigurationArray,
        ...
        $mergeLinkConfigurationArray = \TYPO3\CMS\Core\Utility\ArrayUtility::mergeRecursiveWithOverrule(
        ...
        if (is_array($typoLinkConfigurationArray) && !empty($typoLinkConfigurationArray)) {
            ...
            $linkConfigurationArray[$recordTableName . '.'] = array_merge(
                $linkConfigurationArray[$recordTableName . '.'],
                $typoLinkConfigurationArray
            );
        }
